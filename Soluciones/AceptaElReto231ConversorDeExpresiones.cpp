...
bintree <char> construyeArbol(stack <char> &pila){
	char raiz = pila.top();
	pila.pop();
	if (isdigit(raiz)) return{raiz};
	else{
		bintree<char> arbolDerecha = construyeArbol(pila);
		bintree<char> arbolIzda = construyeArbol(pila);
		return{ arbolIzda, raiz, arbolDerecha };
	}
}
...
bool resuelveCaso() {
	...
	bintree <char> arbol = construyeArbol(pila);
	vector <char> paraCola = arbol.levelorder();
	for (int i = paraCola.size() -1; i >= 0; --i){
		cout << paraCola[i];
	}
	cout << '\n';
		return true;

...
private:
	void arbolCorrecto(Link const& raiz, bool & correcto, int & bolas) {
			if (raiz->elem == '*') {
				correcto = true;
				bolas = 1;
			}
				...
				arbolCorrecto(raiz->left, correcto1, bolas1);
				arbolCorrecto(raiz->right, correcto2, bolas2);
				correcto = correcto1 && correcto2 && (abs(bolas1 - bolas2) <= 1);
				bolas = bolas1 + bolas2;
			}
	}
...
public:
	...
	bool esCorrecto() {
		bool correcto;
		int bolas;
		arbolCorrecto(this->raiz, correcto, bolas);
		return correcto;
	}

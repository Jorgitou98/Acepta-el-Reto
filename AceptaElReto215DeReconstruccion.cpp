...
template <class T>
bintree <T> construyeArbol(queue <T> &recPreorden, vector <T> const& recInorden){
	if (recInorden.empty()) return{};
	else {
		T raiz = recPreorden.front(); // Si la entrada es correcta siempre deberia tener un elem
		recPreorden.pop();
		...
		bintree <T> arbol1 = construyeArbol(recPreorden, v1);
		bintree <T> arbol2 = construyeArbol(recPreorden, v2);
...
bool resuelveCaso() {
	...
	if (preorden == -1)
		return false;
	queue <int> recPreorden;
	while (preorden != -1){
		recPreorden.push(preorden);
		cin >> preorden;
	}
	int inorden;
	...
	bintree <int> arbol = construyeArbol(recPreorden, recInorden);
	vector <int> v3 = arbol.postorder();
